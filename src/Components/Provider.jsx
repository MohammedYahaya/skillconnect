...
const Provider = () => {
  ...
  useEffect(()=>{
    ...
    const works = gsap.utils.toArray(".provider")


      gsap.matchMedia().add("(min-width:481px)",()=>{

      works.forEach((provider,i)=>{
        gsap.to(".provider",{
          opacity:1,
          y:0,
          delay: 1,
  ...
  const ProviderArr = [
    ...
    },
    {
      ...
      subtitle: "Safe, fast, and reliable electrical solutions for homes and businesses."
    },
    {
      img: barber,
  ...
  return (
    <section className="relative min-h-screen bg-white gap-12 flex flex-col justify-center items-center w-full px-6 py-16 lg:px-20">
      
      <h2 className="text-4xl font-bold text-center syncopate-bold">
        Top <span className="text-blue-600">providers</span>
      </h2>
        ...
        {ProviderArr.map((provider, index) => (
          ...
          >
            ...
            <img
              src={star}
              alt="rating"
              className="absolute bottom-[6em] left-5 w-16 h-4 duration-300 group-hover:translate-y-[-4px]"
            />

            {/* Content */}
            <div className="absolute bottom-0 p-5 text-white">
              <h1 className="text-xl font-bold leading-7 mb-2 duration-300 group-hover:translate-y-[-4px]">
                {provider.title}
              </h1>
